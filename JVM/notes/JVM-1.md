...
**TLAB** 的全称是 **Thread Local Allocation Buffer**，即**线程本地分配缓冲区**。它是 JVM 为了提升对象分配效率，在堆内存的新生代` Eden` 区中，为每个线程预先分配的一块**私有缓存区**。

+ 从内存模型而不是垃圾收集的角度，对`Eden`区域继续进行划分，JVM为**每个线程分配了一个私有缓存区域**，它包含在`Eden`空间内。
+ 多线程同时分配内存时，使用TLAB可以避免一系列的非线程安全问题，同时还能够提升内存分配的吞吐量，因此我们可以将这种内存分配方式称之为**快速分配策略**。
+ 所有主流的、基于OpenJDK的JVM实现（如HotSpot、OpenJ9、GraalVM等）都默认开启并支持TLAB。但TLAB本身是HotSpot的实现特性，并非JVM规范强制要求的。



### 8.8.3 TLAB说明

+ 尽管不是所有的对象实例都能够在TLAB中成功分配内存，但JVM确实是将TLAB作为内存分配的**首选路径**。
+ 开发人员可以通过选项 `-XX:+UseTLAB` 开启TLAB（默认开启）。
+ 单个TLAB的大小由 `-XX:TLABSize` 直接控制（单位字节），JVM会根据线程数和`Eden`区大小动态调整默认值。
+ 通过选项 `-XX:TLABWasteTargetPercent`（默认1%）控制所有TLAB累计可浪费空间占`Eden`区的百分比上限，用于限制TLAB的内存浪费，而非直接控制TLAB本身的大小。
+ 当对象在TLAB中分配失败时，JVM会优先尝试为该线程重新申请一个新的TLAB；如果Eden空间不足导致申请失败，才会走加锁/CAS的公共分配路径，或者在极端情况下触发`Minor GC`。



**对象分配一般流程示意图：**

![image-20260812114557231](JVM-1.assets/image-20260812114557231.png)

> 补充：在对象进行首次TLAB分配时，如果分配失败，会重新申请另一块TLAB进行分配。如果申请失败才会回退到`Eden`区进行分配。
...
+ `-XX:+PrintGCDetails`：输出详细的GC处理日志。
  ...
  打印gc简要信息:`-XX:+PrintGC`或` -verbose:gc`

+ `-XX:HandlePromotionFailure`：是否设置空间分配担保。

  该选项在JDK6之后失效，默认开启且无法关闭。
...
年轻代是对象的诞生、成长、消亡的区域，大部分对象在这里创建、使用，最终被垃圾回收器回收。

老年代存放长生命周期的对象，这些对象通常是在多次Minor GC后从Survivor区域**晋升**而来的。普通对象分配在`Eden`区的**TLAB**中；较大对象会直接在Eden区的公共部分分配；如果对象太大，无法在新生代找到足够长的连续空闲空间，JVM会将其直接分配到老年代。

当GC仅发生在年轻代时，称为`Minor GC`。当GC发生在老年代时，称为`Major GC`；若同时回收整个堆（年轻代+老年代+元空间），则称为`Full GC`。通常，`Minor GC`的发生频率比`Major GC`/`Full GC`高得多，即老年代的垃圾回收频率远低于年轻代。
...
**方法区**（Method Area） 是JVM运行时数据区中的一块逻辑上存在的内存区域，用于存储已被虚拟机加载的类信息、常量、静态变量、即时编译器编译后的代码缓存等数据。

它是所有线程共享的内存区域，在JVM启动时创建，逻辑上属于堆的一部分（但具体实现可以独立管理）。



**方法区、永久代、元空间是什么关系？**

方法区 是JVM规范中的概念（逻辑存在），永久代和元空间是HotSpot对这个概念的不同实现（物理载体）。

+ 在JDK8之前，方法区是由永久代实现。（堆内存实现）
+ 在JDK8之后，方法区是由元空间实现。（直接内存实现）



**从线程共享的角度来看**

![image-20260816112148051](JVM-1.assets/image-20260816112148051.png)
...
- **栈**：管理方法的调用和局部变量（基本类型值 + 对象引用）。
- **堆**：存储所有的对象实例（实体数据）。
- **方法区**：存储类的结构和元信息（即对象的“蓝图”）。



![image-20260816113010862](JVM-1.assets/image-20260816113010862.png)
...
![image-20260816173149878](JVM-1.assets/image-20260816173149878.png)
...
+ 它在 JVM 启动时被创建，其实际的物理内存空间可以和Java堆区一样是不连续的。

+ 方法区的大小可选择固定大小或可扩展，通过 JVM 参数进行控制：

  - JDK 7 及之前：`-XX:PermSize` / `-XX:MaxPermSize`
  - JDK 8 及之后：`-XX:MetaspaceSize` / `-XX:MaxMetaspaceSize`

+ 方法区的大小决定了系统能保存多少个类。如果加载的类数量过多（如大量第三方 JAR 包、Tomcat 部署过多工程、大量动态生成的反射类/代理类），导致方法区内存不足，JVM 会抛出 OOM 错误：

  - JDK 7 及之前：`java.lang.OutOfMemoryError: PermGen space`

  - JDK 8 及之后：`java.lang.OutOfMemoryError: Metaspace`

+ 方法区也会进行垃圾回收（废弃常量、无用类），但回收条件苛刻、频率较低。

+ 当 JVM 进程正常关闭时，方法区占用的内存会被操作系统回收。
...
+ 在jdk7及之前，习惯上把方法区成为永久代。从jdk8开始，使用元空间取代了永久代。

+ 本质上，方法区和永久代并不等价。对HotSpot而言，方法区是《Java虚拟机规范》中定义的逻辑概念，对于如何实现方法区《规范》并不做统一要求。

  > 现在来看，使用永久代不是并不是一个好的实现。因为永久代`-XX:MaxPermSize`通常设置比较小，内存回收比较困难以及碎片化问题导致Java程序更容易OOM。
...
<img src="JVM-1.assets/image-20260821211318310.png" alt="image-20260821211318310" style="zoom:80%;" />
